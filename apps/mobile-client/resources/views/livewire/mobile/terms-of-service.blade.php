<section class="safe-x safe-pb mx-auto flex min-h-full w-full max-w-md flex-col gap-5 py-6">
    ...
    <x-mobile.card title="Summary" description="Important points before accepting.">
        <ul class="grid gap-3">
            @forelse ($policy['summary'] as $summary)
                <li wire:key="terms-summary-{{ $loop->index }}" class="rounded-lg border border-app-line bg-app-bg p-4 text-sm leading-6 text-app-ink dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100">
                    {{ $summary }}
                </li>
            @empty
    ...
    <div class="grid gap-3 sm:grid-cols-2">
        <a href="{{ route('mobile.privacy') }}" wire:navigate class="inline-flex min-h-12 items-center justify-center rounded-lg border border-app-line bg-app-surface px-4 text-sm font-semibold text-app-ink shadow-sm transition hover:bg-app-bg dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100 dark:hover:bg-zinc-800">
            Privacy Policy
        </a>
