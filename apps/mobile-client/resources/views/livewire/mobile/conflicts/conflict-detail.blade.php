<section class="safe-x safe-pb flex min-h-full flex-col gap-5 py-6">
    ...
    @if ($statusMessage)
        <div @class([
            ...
            'border-emerald-200 bg-emerald-50 text-emerald-800   ' => $statusVariant === 'success',
            'border-amber-200 bg-amber-50 text-amber-900   ' => $statusVariant === 'warning',
            'border-sky-200 bg-sky-50 text-sky-800   ' => $statusVariant === 'info',
        ])>
            {{ $statusMessage }}
        </div>
    @endif
    ...
    <x-mobile.card title="Sync action" description="The local mutation that could not be applied automatically.">
        <dl class="grid gap-3">
            @foreach ($summaryRows as $row)
                <div wire:key="conflict-summary-{{ str($row['label'])->slug() }}" class="grid grid-cols-[8rem_1fr] gap-3 border-b border-app-line pb-3 last:border-b-0 last:pb-0 ">
    ...
    <div class="grid gap-3 sm:grid-cols-2">
        <x-mobile.card title="Local payload" description="The offline change stored on this device.">
            <pre class="max-h-80 overflow-auto rounded-lg bg-app-bg p-3 text-xs leading-5 text-app-ink  ">{{ $localPayloadJson }}</pre>
        </x-mobile.card>

        <x-mobile.card title="Remote payload" description="The server version returned with the conflict.">
            <pre class="max-h-80 overflow-auto rounded-lg bg-app-bg p-3 text-xs leading-5 text-app-ink  ">{{ $remotePayloadJson }}</pre>
        </x-mobile.card>
    </div>

    <x-mobile.card title="Conflict payload" description="Raw conflict metadata from the sync response.">
        <pre class="max-h-80 overflow-auto rounded-lg bg-app-bg p-3 text-xs leading-5 text-app-ink  ">{{ $serverPayloadJson }}</pre>
    </x-mobile.card>
