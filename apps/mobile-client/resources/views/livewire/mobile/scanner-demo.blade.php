    ...
    >
        ...
        @if ($pendingScanId)
            <div class="mt-3 rounded-lg border border-sky-200 bg-sky-50 p-4  ">
                <p class="text-sm font-semibold text-sky-950 ">Pending scan session</p>
                <p class="mt-1 break-words text-sm text-sky-900 ">{{ $pendingScanMode }} · {{ $pendingScanId }}</p>
            </div>
        @endif
            ...
            <div class="grid gap-4">
                ...
                <div class="grid gap-3">
                    @forelse ($scannerActions as $scannerAction)
                        ...
                        >
                            <div>
                                <p class="text-base font-semibold text-app-ink ">{{ $scannerAction['label'] }}</p>
                                <p class="mt-1 text-sm leading-5 text-app-muted ">{{ $scannerAction['description'] }}</p>
                            </div>
        ...
        @if ($latestData)
            <div class="grid gap-3">
                <div class="flex items-start justify-between gap-4">
                    <div class="min-w-0">
                        <p class="break-words text-lg font-semibold text-app-ink ">{{ $latestData }}</p>
                        <p class="mt-1 text-sm text-app-muted ">{{ $latestScannedAt }}</p>
                    </div>
                ...
                <p class="break-words rounded-lg border border-dashed border-app-line bg-app-bg px-3 py-2 text-sm font-medium text-app-muted   ">
                    {{ $latestData }}
                </p>
            </div>
            ...
            <div class="grid gap-3">
                @forelse ($scanHistory as $scan)
                    ...
                    >
                        <div class="flex items-start justify-between gap-4">
                            <div class="min-w-0">
                                <p class="break-words text-base font-semibold text-app-ink ">{{ $scan['data'] }}</p>
                                <p class="mt-1 text-sm text-app-muted ">{{ $scan['scanned_at'] }}</p>
                            </div>
        ...
        <x-slot:footer>
            <div class="grid gap-2">
                @forelse ($formatOptions as $format)
                    <p wire:key="scanner-format-{{ $format['value'] }}" class="text-sm leading-5 text-app-muted ">
                        <span class="font-semibold text-app-ink ">{{ $format['label'] }}:</span>
                        {{ $format['description'] }}
                    </p>
                @empty
                    <p class="text-sm text-app-muted ">No barcode formats are configured.</p>
                @endforelse
            </div>
        </x-slot:footer>
    </x-mobile.card>
