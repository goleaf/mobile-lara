    ...
    <x-mobile.card title="Permanent deletion" description="This action is destructive.">
        <div class="grid gap-4">
            ...
            <fieldset class="grid gap-3">
                <legend class="text-sm font-semibold text-app-ink ">Confirmation method</legend>
                ...
                <label class="flex items-start gap-3 rounded-lg border border-app-line bg-app-bg p-4  ">
                    <input
                        ...
                        class="mt-1 size-4 border-app-line text-app-accent focus:ring-app-accent  "
                    >
                    <span class="min-w-0">
                        <span class="block text-sm font-semibold text-app-ink ">Password confirmation</span>
                        <span class="mt-1 block text-sm leading-5 text-app-muted ">Use the password for the signed-in server account.</span>
                    </span>
                </label>
                ...
                <label class="flex items-start gap-3 rounded-lg border border-app-line bg-app-bg p-4  ">
                    ...
                    >
                    <span class="min-w-0">
                        <span class="block text-sm font-semibold text-app-ink ">Biometric confirmation</span>
                        <span class="mt-1 block text-sm leading-5 text-app-muted ">Use NativePHP biometrics on this device.</span>
                    </span>
                </label>
            </fieldset>
            ...
            @if ($confirmationMethod === 'password')
                ...
                />
            @else
                <div class="grid gap-3 rounded-lg border border-app-line bg-app-bg p-4  ">
                    <div class="flex items-center justify-between gap-4">
                        <div class="min-w-0">
                            <p class="text-sm font-semibold text-app-ink ">Native biometric check</p>
            ...
            <label class="flex items-start gap-3 rounded-lg border border-app-line bg-app-bg p-4  ">
                <input
                    ...
                    class="mt-1 size-5 rounded border-app-line text-red-600 focus:ring-red-500  "
                >
            ...
            @if ($deletionRequest !== [])
                <dl class="grid gap-2 rounded-lg border border-dashed border-app-line bg-app-bg p-4 text-sm  ">
                    <div class="flex items-center justify-between gap-4">
                        <dt class="text-app-muted ">Endpoint</dt>
                        <dd class="text-right font-mono text-xs font-semibold text-app-ink ">{{ $deletionRequest['server_endpoint'] }}</dd>
                    </div>
                    <div class="flex items-center justify-between gap-4">
                        <dt class="text-app-muted ">Confirmed by</dt>
                        <dd class="text-right font-semibold text-app-ink ">{{ $deletionRequest['confirmed_by'] }}</dd>
                    </div>
                </dl>
            @endif
        </div>
