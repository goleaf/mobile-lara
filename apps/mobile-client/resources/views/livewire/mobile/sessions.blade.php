...
<section class="safe-x safe-pb mx-auto flex min-h-full w-full max-w-md flex-col gap-5 py-6">
    ...
    <x-mobile.card title="Current device session" description="Local session details for this app install.">
        <div class="grid gap-4">
            <div class="flex items-start justify-between gap-4 rounded-lg border border-app-line bg-app-bg p-4  ">
                <div class="flex min-w-0 items-start gap-3">
                    <span class="grid size-11 shrink-0 place-items-center rounded-lg bg-app-ink text-white  ">
                    ...
                    <div class="min-w-0">
                        <p class="text-base font-semibold text-app-ink ">{{ $currentSession['device_name'] }}</p>
            ...
            <dl class="grid gap-3">
                <div class="flex items-center justify-between gap-4 rounded-lg border border-app-line bg-app-surface px-4 py-3  ">
                    <dt class="text-sm font-medium text-app-muted ">Last login time</dt>
                    <dd class="max-w-[58%] text-right text-sm font-semibold text-app-ink ">
                        {{ $currentSession['last_login_label'] }}
                    </dd>
                </div>

                <div class="flex items-center justify-between gap-4 rounded-lg border border-app-line bg-app-surface px-4 py-3  ">
                    <dt class="text-sm font-medium text-app-muted ">App version</dt>
                    <dd class="max-w-[58%] text-right text-sm font-semibold text-app-ink ">
                        {{ $currentSession['app_version'] }}
                        <span class="font-medium text-app-muted ">({{ $currentSession['app_version_code'] }})</span>
                    </dd>
                </div>
            </dl>
        </div>
        ...
        <x-mobile.card title="Remote sessions" description="Placeholder rows for the future sessions API.">
            <div class="grid gap-3">
                @forelse ($remoteSessions as $remoteSession)
                    <article wire:key="remote-session-{{ $remoteSession['id'] }}" class="rounded-lg border border-dashed border-app-line bg-app-bg p-4  ">
                        <div class="flex items-start justify-between gap-4">
                            <div class="min-w-0">
                                <p class="text-base font-semibold text-app-ink ">{{ $remoteSession['device_name'] }}</p>
                                <p class="mt-1 text-sm leading-5 text-app-muted ">{{ $remoteSession['platform'] }}</p>
                            </div>
                        ...
                        <dl class="mt-4 grid gap-2 text-sm">
                            <div class="flex items-center justify-between gap-4">
                                <dt class="text-app-muted ">Last active</dt>
                                <dd class="text-right font-medium text-app-ink ">{{ $remoteSession['last_active_label'] }}</dd>
                            </div>

                            <div class="flex items-center justify-between gap-4">
                                <dt class="text-app-muted ">Source</dt>
                                <dd class="text-right font-mono text-xs font-semibold text-app-ink ">{{ $remoteSession['source'] }}</dd>
                            </div>
                        </dl>
                    </article>
                @empty
