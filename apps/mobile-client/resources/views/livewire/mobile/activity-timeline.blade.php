                    ...
                    <article wire:key="activity-timeline-{{ $row['key'] }}" class="grid grid-cols-[0.75rem_1fr] gap-3">
                        <div class="pt-1.5">
                            <span
                                @class([
                                    ...
                                    'bg-emerald-500 ' => $row['variant'] === 'success',
                                    'bg-red-500 ' => $row['variant'] === 'danger',
                                    'bg-app-accent ' => $row['variant'] === 'accent',
                                    'bg-app-muted ' => ! in_array($row['variant'], ['success', 'danger', 'accent'], true),
                                ])
                            ></span>
                        </div>
                        ...
                        <div class="rounded-lg border border-app-line bg-app-bg p-3  ">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="break-words text-sm font-semibold text-app-ink ">{{ $row['title'] }}</p>
                                    <p class="mt-1 text-xs font-medium text-app-muted ">{{ $row['time'] }}</p>
                                </div>
                            ...
                            <p class="mt-2 whitespace-pre-line break-words text-sm leading-5 text-app-ink ">{{ $row['message'] }}</p>

                            @if ($row['meta'])
                                <p class="mt-2 break-words text-xs leading-5 text-app-muted ">{{ $row['meta'] }}</p>
                            @endif
                        </div>
                    </article>
                @empty
