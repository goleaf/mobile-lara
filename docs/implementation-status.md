...
| Mobile tests | tested | `php artisan test --compact` passes in `apps/mobile-client` with 435 tests / 3467 assertions. |
...
1. Continue migrating lower-level mobile actions and NativePHP service calls
   behind `MobileAccessPolicy`, especially record mutations, attachments,
   voice notes, check-ins, and offline queue writes.
