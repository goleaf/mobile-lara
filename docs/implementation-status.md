...
| Share | partial | Service exists, and file-manager plus developer debug share actions are gated by cached `native_share` policy. Diagnostics/report sharing policy remains incomplete. |
| Browser | partial | Service exists, and developer debug browser actions are gated by cached `native_browser` policy. Feature-specific support/legal/billing policy remains incomplete. |
...
| Mobile tests | tested | `php artisan test --compact` passes in `apps/mobile-client` with 455 tests / 3650 assertions. |
