...
| Area | Current state |
| --- | --- |
| Root application | The repository root is now a monorepo shell with docs, contracts, scripts, and workspace command wrappers only. It is not a Laravel runtime. |
...
| Root Laravel app cleanup | tested | The former root mobile transition app has been removed from tracked source. API-only mobile auth coverage now lives in `apps/mobile-client`. |
...
| Environment examples | tested | Root `.env.example` documents that runtime env files live in each app; `apps/api-admin` and `apps/mobile-client` keep their app-specific `.env.example` files. |
