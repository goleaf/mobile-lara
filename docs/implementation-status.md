...
| Support contract | tested | `v1-support.md` defines implemented requester-scoped ticket list, create, detail, message create, attachment metadata, diagnostic references, tenant isolation, and audit behavior. Admin support queues, stored attachments, support-agent replies, assignment/status controls, and offline draft replay remain pending. |
...
| Support/admin access limits | partial | Platform-admin diagnostics review is implemented with policy checks. Role-specific support queues, support-agent scoping, and ticket-linked diagnostics remain pending. |
...
| Feature | Status | Notes |
| --- | --- | --- |
| Support tickets/messages | tested | Requester-scoped mobile ticket list/create/detail and message-create endpoints are implemented with tenant isolation, permission checks, and audit history. Admin queues and support-agent replies remain pending. |
...
| Assignment/status/priority | partial | Mobile ticket creation captures priority and read-only assignment/status payloads. Admin assignment/status controls remain pending. |
| Support audit | tested | Mobile ticket create and message create events write security audit history. Admin/support access audit views remain pending. |
...
| API/admin tests | tested | `php artisan test --compact` passes in `apps/api-admin` with 147 tests / 1236 assertions. |
| API/admin frontend build | tested | `npm run build` passes in `apps/api-admin`. |
| API routes verification | tested | `php artisan route:list --except-vendor` shows 45 app routes including app-version, auth, bootstrap, config, contracts, diagnostics, features, notifications, support, status, sync, tenant context, tenant invitation, records routes, and the admin sync monitor; `--path=api/v1/mobile` shows 40 mobile API routes. |
| Admin navigation verification | tested | Admin dashboard smoke coverage exists, and the sync monitor route/view is covered by feature tests; browser-level verification remains future. |
