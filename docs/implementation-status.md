...
| Feature | Status | Notes |
| --- | --- | --- |
| Complete Laravel app under `apps/mobile-client` | tested | Root mobile app was copied into `apps/mobile-client` with Composer/NPM lockfiles; `composer validate`, 55 route verification, full Pest suite, Pint, Vite build, and NativePHP plugin validation pass. |
...
| Mobile support ticket list/create/detail | partial | API endpoints and `MobileSupportApiService` exist. Mobile Livewire ticket screens and offline draft replay remain pending. |
...
| Mobile tests | tested | `php artisan test --compact` passes in `apps/mobile-client` with 498 tests / 3896 assertions. |
