...
| Mobile routes | The root transition app remains as a transition mirror; `apps/mobile-client/routes/web.php` now exposes 59 named `mobile.*` routes, including API-backed support ticket list/create/detail screens and the billing screen. |
...
| API/admin tests | tested | `php artisan test --compact` passes in `apps/api-admin` with 159 tests / 1312 assertions. |
...
| Mobile tests | tested | `php artisan test --compact` passes in `apps/mobile-client` with 513 tests / 3959 assertions. |
