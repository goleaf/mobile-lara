...
| Area | Current state |
| --- | --- |
| Root application | The repository root is now a monorepo shell with docs, contracts, scripts, and workspace command wrappers only. It is not a Laravel runtime. The local `mobile-lara.test` Herd link points to `apps/mobile-client`. |
| Requested monorepo paths | `apps/api-admin` and `apps/mobile-client` are the only runtime Laravel applications. |
| API routes | `apps/api-admin/routes/api.php` exposes versioned routes at `GET /api/v1/mobile/status`, `GET /api/v1/mobile/contracts`, public `GET /api/v1/mobile/app-version`, auth endpoints, authenticated `GET /api/v1/mobile/bootstrap`, `GET /api/v1/mobile/config`, `GET /api/v1/mobile/features`, `GET /api/v1/mobile/billing/subscription`, `POST /api/v1/mobile/diagnostics`, notification inbox/read/delete/push-token endpoints, support ticket/message endpoints, sync bootstrap/pull/push/acknowledge endpoints, `GET /api/v1/mobile/tenants`, `POST /api/v1/mobile/tenants/current`, authenticated tenant invitation list/accept/decline endpoints, and tenant-scoped records list/create/detail/update/archive/restore endpoints. |
| Mobile routes | `apps/mobile-client/routes/web.php` exposes 59 named `mobile.*` routes, including API-backed support ticket list/create/detail screens and the billing screen. |
| Active database | API/admin migrations now include users, framework tables, mobile device sessions, hashed mobile access/refresh tokens, security audit events, tenants, tenant-user memberships, feature flag tables, remote config tables, app-version policy tables, mobile diagnostic reports, mobile sync events, mobile notifications, mobile push tokens, mobile support tickets/messages, and tenant-scoped record/category/tag/note/attachment/activity tables. Broader control-plane domain schema remains pending. |
...
| Root monorepo structure | tested | The root is a monorepo shell; `apps/api-admin` and `apps/mobile-client` are the only Laravel apps. |
...
| Root scripts | tested | Root Composer/NPM manifests now delegate install, test, format, build, migration, and NativePHP helper commands into `apps/api-admin` and `apps/mobile-client`. |
| Environment examples | tested | Root `.env.example` documents that runtime env files live in each app; `apps/api-admin` and `apps/mobile-client` keep their app-specific `.env.example` files with `https://mobile-lara-api-admin.test` and `https://mobile-lara.test` defaults. |
...
| Feature | Status | Notes |
| --- | --- | --- |
| Complete Laravel app under `apps/mobile-client` | tested | Root mobile app was copied into `apps/mobile-client` with Composer/NPM lockfiles; `composer validate`, 59 named mobile route verification, full Pest suite, Pint, Vite build, and NativePHP plugin validation pass at the last full quality checkpoint. |
...
| Tailwind mobile styling | tested | Tailwind v4/SCSS entrypoint and mobile design tokens build through Vite in `apps/mobile-client`; the mobile UI is light-only, dark variants are removed, and shared light tokens now include soft/raised surfaces for the mobile content canvas and controls. |
| Mobile-first layout and safe-area shell | tested | Shared layout, mobile components, safe-area shell, bottom navigation, the single shell-header rule, refactored scrollable content canvas, larger touch targets, stable bottom labels, and centered Create tab are covered by component tests, Playwright viewport checks, and the mobile Vite build. |
...
| Bottom navigation | tested | `<x-mobile.bottom-navigation>` exists with enlarged icon targets, a taller shell footer track, centered Create action, visible labels with narrow-screen truncation, active indicators, Playwright viewport checks, and shell tests; feature-gated navigation is not API-controlled yet. |
...
| Reusable mobile UI components | tested | Components exist and are covered by mobile UI component tests; shared cards, buttons, inputs, selects, textareas, modals, sheets, toasts, skeletons, empty states, and error states now share the polished light-only touch/focus/surface treatment, while shared page headers render only compact back/action controls so page titles are not duplicated. |
...
| Notifications contract | tested | `v1-notifications.md` defines implemented tenant/user-safe inbox, push-token registration/revocation, unread count, read/read-all, and delete behavior. Admin notification center, campaigns, delivery providers, and mobile push registration UI remain pending. |
...
| Sync contract | tested | `v1-sync.md` defines implemented sync bootstrap, records-only push, records-only pull, acknowledgement, idempotent replay, sync event storage, conflict behavior, and platform-admin sync monitoring. Non-record collection replay remains pending. |
| Support contract | tested | `v1-support.md` defines implemented requester-scoped ticket list, create, detail, message create, platform-admin queue triage, requester-visible admin replies, attachment metadata, diagnostic references, tenant isolation, and audit behavior. Stored attachment files, role-specific support-agent scoping, notification orchestration, and offline draft replay remain pending. |
| Billing contract | tested | `v1-billing.md` defines subscription/plan presentation behavior; `GET /billing/subscription` returns current tenant subscription state and plan hints, `/admin/billing` edits provider-neutral tenant billing payloads with audit, and the mobile Billing screen displays live or cached subscription state. Provider billing, invoices, and usage event writes remain pending. |
| Reports contract | documented | `v1-reports.md` defines permission-safe report summaries; API is not implemented. |
| Diagnostics contract | tested | `v1-diagnostics.md` defines mobile-local diagnostics export/share, authenticated upload behavior, and platform-admin diagnostics review. Support-ticket linking and role-specific support-agent queues remain pending. |
...
| Policies for API/admin | tested | Policies are registered for current mobile control-plane resources, including sync events, and Livewire edit/save/restore/review actions use policy authorization. Broader resource policies remain pending as new modules are implemented. |
...
| Protected API routes | partial | Auth, bootstrap, tenant list/switch, profile, diagnostics, notifications, support, sync, and records routes are mobile-token protected; broader resource permission middleware/policies remain pending. |
...
| Mobile permission-aware UI | tested | Permission settings/center exists for NativePHP device permissions and now uses `MobileAccessPolicy` before offering camera, microphone, location, notification, file, or biometric prompts. `MobileAccessPolicy` also consumes cached Admin/API bootstrap permissions to hide blocked shortcuts and route-block records, notifications, billing, sync conflicts, media/files, scanner, and location screens; record create/update/archive/delete, bulk mutations, attachment management, attachment sharing, profile sharing, record-detail sharing, media-gallery sharing, support-center browser handoff, billing screen refresh/portal actions, voice-note recording callbacks, local voice-note save/delete, voice-note upload queue placeholders, NativePHP location callbacks, local check-in creation, media capture callbacks, file manager read/write/copy/move/import/export/delete/share actions, scanner capture callbacks, saved scan-history deletes/clears, notification read/open/read-all actions, manual sync, conflict resolution actions, offline-first queue writes, diagnostics share actions, and developer debug native wrapper calls/callbacks now deny direct Livewire/service calls before local SQLite writes, native share/location/media/scanner/browser/device/dialog/storage handoff, file deletes, imports/exports, scan-history deletes, notification timestamp writes, billing portal handoff, sync timestamp writes, conflict queue status changes, diagnostics native handoff, or offline queue writes. Remaining report native share flows still need feature-specific policy gates. |
...
| Mobile config store/cache | tested | `MobileRemoteConfigStore` reads the cached bootstrap envelope without creating a settings row, exposes typed `support`, `legal`, `sync`, `uploads`, `dashboard`, and `app_lock` values, validates URLs safely, and falls back to bundled defaults when config is missing or malformed. |
| Offline defaults | tested | API/admin returns defaults-used, freshness, compatibility, and fallback metadata; mobile-local config reads now use bundled safe defaults and reject unsafe scalar/url overrides. Rich stale-cache freshness UI remains pending. |
| Config-driven sync/upload/legal/support behavior | tested | Cached remote config now drives dashboard quick-stat widgets, sync settings summary, storage upload-limit hints, support center URL/diagnostics state, legal link browser actions, and app-lock policy copy. |
...
| Notification preferences | tested | Bootstrap returns tenant notification preferences, quiet hours, push-registration hints, unread count, fail-closed no-tenant behavior, and notification policy version metadata. |
...
| Unread notification count | tested | Bootstrap returns the user-scoped unread server inbox count for the current tenant. |
...
| Feature | Status | Notes |
| --- | --- | --- |
| API/admin search | partial | `/admin/records` implements tenant/status/archive filters plus search across record, tenant, category, and tag fields; broader API search, recent searches, saved views, and cross-module search remain pending. |
...
| Sync conflict tracking | tested | partial | Server sync events store accepted/rejected/conflict outcomes with idempotency keys, stale record versions return conflict payloads, admin sync monitoring can review conflict/rejection payloads, and local conflict fields/screens remain available. Conflict resolution workflows remain pending. |
| Admin sync monitoring | tested | n/a | `/admin/mobile/sync` lists accepted/rejected/conflict/unacknowledged sync events, supports search/outcome filtering, and shows replay detail payloads under platform-admin policy. |
...
| Browser | partial | Service exists, and support-center, legal, billing portal, and developer debug browser actions are gated by cached `native_browser` policy. Provider-hosted billing and deeper legal/provider workflows remain pending. |
...
| Push notifications | partial | Local notifications exist, API push-token register/revoke endpoints exist, and the mobile API service can call them. Native push permission/token capture and registration UI remain pending. |
...
| Attach to records/support | partial | Support ticket/message endpoints accept attachment metadata, but voice upload/storage and record/support media attachment replay remain pending. |
...
| Feature | Admin/API Status | Mobile Status | Notes |
| --- | --- | --- | --- |
| Notifications | tested | partial | Server tenant/user-safe inbox endpoints exist and local notification inbox exists. Admin notification creation/orchestration remains pending. |
| Notification preferences | tested | partial | Bootstrap returns tenant notification preferences and unread count from API-owned notification state; mobile settings exist. |
| Push token registration/revocation | tested | partial | API endpoints hash/register/revoke push tokens when tenant policy allows push. Mobile service calls exist; native permission/token capture and registration UI remain pending. |
...
| Delivery/open tracking | partial | partial | Server stores delivery/open timestamps but external provider delivery and mobile open tracking integration remain pending. |
| Mark read/all read/delete | tested | tested | API read, read-all, and soft-delete endpoints exist; local read/open/read-all behavior exists and cached notification policy gates local actions. |
| Deep links | partial | partial | API returns notification `deep_link` values and Native config has scheme; full mobile deep-link handling remains incomplete. |

## Phase 22 - Security And Privacy

| Feature | Status | Notes |
| --- | --- | --- |
| Server audit logic | partial | Auth, tenant, feature/config/version, records, diagnostics, notifications, support ticket/message actions, and sync replay/acknowledgement events write security audit history; broader support/report/admin audit views remain pending. |
...
| Secure token handling | tested | API/admin issues hashed access/refresh tokens and protects mobile routes through bearer-token middleware; the mobile client stores returned token sets through `MobileTokenStore` with NativePHP secure storage as default and session fallback for tests/development. Broader app-lock and step-up policy remains partial. |
...
| Support/admin access limits | partial | Platform-admin diagnostics review and platform-admin support queue triage are implemented with policy checks. Role-specific support-agent scoping and ticket-linked diagnostics remain pending. |

## Phase 23 - Billing And Subscriptions

| Feature | Status | Notes |
| --- | --- | --- |
| Plans/subscriptions | partial | `MobileSubscriptionResolver` reads tenant `subscription_state` and mobile-safe billing settings for bootstrap, `/billing/subscription`, feature plan gates, `/admin/billing`, and the mobile Billing screen. Provider plans/subscriptions remain pending. |
| Invoice placeholders | not started | Documented only. |
| Usage events | not started | Documented only. |
| Admin billing screens | tested | `/admin/billing` lists tenants, searches/filters subscription states, selects tenant billing detail, edits subscription state, plan metadata, trial date, portal URL, limits JSON, usage JSON, preserves unrelated tenant settings, and writes `admin_billing_updated` audit events. |
...
| Mobile plan/status display | tested | `/billing` renders live `GET /billing/subscription` data, displays plan, state, trial ending, limits/usage, available actions, portal status, and falls back to cached bootstrap subscription data when the API is unavailable. |
| Trial/expired/suspended behavior | partial | API/bootstrap and mobile Billing UI expose subscription state, feature-limited flags, cached-state warnings, and safe recovery actions. Provider-driven trial expiry, invoices, payment recovery, and notifications remain pending. |

## Phase 24 - Support System

| Feature | Status | Notes |
| --- | --- | --- |
| Support tickets/messages | tested | Requester-scoped mobile ticket list/create/detail/message endpoints, mobile screens, platform-admin queue review, status/priority/assignment controls, and requester-visible admin replies are implemented with tenant isolation, permission checks, and audit history. |
| Support attachments | partial | Foundation support endpoints accept attachment metadata only. File upload, storage, scanning, retention, preview, and download remain pending. |
| Admin support panel | tested | `/admin/support` lets platform admins search/filter support tickets, review requester-safe detail, assign support users, update status/priority, and send requester-visible replies. Role-specific support-agent queues remain pending. |
| Assignment/status/priority | tested | Mobile ticket creation captures priority and read-only assignment/status payloads; `/admin/support` updates status, priority, assignment, and close timestamps with audit history. |
| Support audit | tested | Mobile ticket create/message create plus admin ticket update/reply events write security audit history. Broader support audit reporting views remain pending. |
| Mobile support ticket list/create/detail | tested | API endpoints, `MobileSupportApiService`, and Livewire mobile ticket list/create/detail/reply screens exist with support feature/permission gates, API error handling, route coverage, and focused Pest coverage. Offline draft replay remains pending. |
...
| Sync/offline/upload limits | partial | Mobile settings now renders cached sync policy and upload-limit guidance from remote config. Records-only server sync endpoints and admin sync monitoring exist; upload acceptance remains pending. |
...
| API diagnostics upload | tested | `POST /api/v1/mobile/diagnostics` accepts authenticated snapshots for the current active tenant, re-applies server-side redaction, stores report metadata, and writes audit history. |
| Admin diagnostics review | tested | `/admin/mobile/diagnostics` lets platform admins search and review redacted diagnostics reports without exposing mobile-user emails or raw support secrets. |
...
| Notifications/billing/support/reports/security docs | partial | Product-level docs exist; implementation-specific docs now cover notifications, support, billing, diagnostics, and security/audit slices as implemented. Reports and provider-backed billing documentation remain future work. |
...
| API/admin tests | tested | `php artisan test` passes in `apps/api-admin` with 166 tests / 1400 assertions. |
...
| Final git status | tested | This consolidation is committed as a single structure cleanup slice; final worktree status is checked after commit. |
...
| --- | --- | --- |
| Root transition app removal | resolved | The root Laravel transition app has been removed; runtime code now lives only in `apps/api-admin` and `apps/mobile-client`. |
