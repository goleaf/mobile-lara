...
| Mobile routes | The root transition app remains as a transition mirror; `apps/mobile-client/routes/web.php` now exposes 58 named `mobile.*` routes, including API-backed support ticket list/create/detail screens. |
...
| Feature | Status | Notes |
| --- | --- | --- |
| Complete Laravel app under `apps/mobile-client` | tested | Root mobile app was copied into `apps/mobile-client` with Composer/NPM lockfiles; `composer validate`, 58 named mobile route verification, full Pest suite, Pint, Vite build, and NativePHP plugin validation pass at the last full quality checkpoint. |
...
| API/admin tests | tested | `php artisan test --compact` passes in `apps/api-admin` with 153 tests / 1274 assertions. |
...
| Mobile tests | tested | `php artisan test --compact` passes in `apps/mobile-client` with 509 tests / 3938 assertions. |
| Mobile frontend build | tested | `npm run build` passes in `apps/mobile-client`. |
| Mobile navigation verification | tested | `php artisan route:list --json` confirms 58 named `mobile.*` routes and route tests cover authenticated/guest rendering plus the support ticket routes. Browser/native manual verification remains future. |
