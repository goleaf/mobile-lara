...
The notifications model is explicit: notification targeting, delivery policy,
push behavior, in-app inbox, read/unread state, deep links, preferences,
...
The billing and plan model is explicit: plan-based access, trial behavior,
active/expired/suspended subscription states, plan limits, feature-flag
entitlement ceilings, mobile unavailable-feature states, and manual admin
billing controls stay Admin/API-authoritative, tenant-scoped, auditable, and
provider-neutral.

The reporting model is explicit: admins, tenant admins, and mobile users see
only role-appropriate reporting outcomes; privacy boundaries, date ranges,
exports, feature usage, sync health, notification, support, and billing reports
stay tenant-scoped, permission-aware, privacy-safe, auditable, and
Admin/API-authoritative.

The native feature strategy is explicit: NativePHP capabilities are wrapped
behind logical product boundaries with browser/development fallbacks,
...
The scanner model is explicit: QR/barcode scan-to-search, scan-to-create,
scan-to-validate, scan history, offline scanning, invalid scan handling,
